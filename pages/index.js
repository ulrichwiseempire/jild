...
export default function Feed() {
  ...
  // Gestion séparée : Média principal (Image/Vidéo) + Musique de fond
    ...
    <div className="min-h-screen bg-black text-[#EFF3F4] pb-20 max-w-md mx-auto border-x border-[#2F3336]">
      ...
      <div className="flex gap-3 p-3 overflow-x-auto border-b border-[#2F3336]">
        ...
        {stories.map((story) => (
          <div key={story.id} className="flex flex-col items-center flex-shrink-0">
            ...
            <span className="text-[10px] mt-1 text-[#71767B] truncate w-14 text-center">@{story.profiles?.username || 'anonyme'}</span>
          </div>
        ))}
      </div>
      ...
      <div className="p-4 border-b border-[#2F3336]">
        <form onSubmit={handleCreatePost}>
          ...
          {/* Aperçus médias */}
          {visualPreview && (
            <div className="relative mb-2 rounded-2xl overflow-hidden border border-[#2F3336]">
              <button type="button" onClick={() => { setVisualFile(null); setVisualPreview(null) }} className="absolute top-2 right-2 bg-black/70 rounded-full p-1 z-10">
                <X className="w-4 h-4 text-white" />
              </button>
              {visualType === 'image' && <img src={visualPreview} className="w-full max-h-60 object-cover" />}
              {visualType === 'video' && <video src={visualPreview} controls className="w-full max-h-60" />}
            </div>
          )}

          {audioFile && (
            <div className="flex items-center justify-between p-2 mb-3 bg-[#16181C] rounded-xl border border-[#2F3336] text-xs text-[#1D9BF0]">
              <span className="flex items-center gap-1 truncate"><Music className="w-4 h-4" /> {audioName}</span>
              <button type="button" onClick={() => setAudioFile(null)}><X className="w-4 h-4 text-white" /></button>
            </div>
          )}
          ...
          <div className="flex justify-between items-center pt-2 border-t border-[#2F3336]">
            ...
            <button
              ...
              className="bg-[#1D9BF0] hover:bg-blue-500 disabled:opacity-50 text-white font-bold px-4 py-1.5 rounded-full text-sm"
            >
              {loading ? '...' : 'Poster'}
            </button>
          </div>
