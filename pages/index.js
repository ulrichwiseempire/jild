  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between font-sans pb-16">
      ...
      <main className="max-w-2xl mx-auto w-full p-4 flex-grow space-y-6">
        ...
        {/* VOLET PARAMÈTRES ET ACTIVITÉ (STYLE INSTAGRAM) */}
        {isSettingsOpen && (
          <div className="fixed inset-0 bg-black/80 z-50 flex justify-end">
            <div className="bg-slate-900 w-full max-w-sm h-full p-5 flex flex-col justify-between border-l border-slate-800">
              <div className="space-y-6">
                ...
                <div className="space-y-4 text-sm text-slate-300">
                  ...
                  <div className="w-full text-left py-2 flex items-center gap-3 opacity-50 cursor-not-allowed">🔖 Enregistré</div>
                  <div className="w-full text-left py-2 flex items-center gap-3 opacity-50 cursor-not-allowed">📊 Votre activité</div>
                  <div className="w-full text-left py-2 flex items-center gap-3 opacity-50 cursor-not-allowed">🔔 Notifications</div>
                </div>
              </div>
            ...
            return (
              <article key={story.id} className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <Link href={story.user_id ? `/profile/${story.user_id}` : '#'} className="flex items-center gap-3 hover:opacity-80">
                    {story.avatar_url ? (
      ...
      {/* BARRE DE NAVIGATION EN BAS (STYLE INSTAGRAM OFFICIEL) */}
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-950 border-t border-slate-800 py-3 px-6 z-40">
        <div className="max-w-md mx-auto flex justify-between items-center text-white">
          ...
          {/* VIDÉOS / REELS */}
          <button className="hover:opacity-70 transition opacity-60">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          {/* ICÔNE MESSAGES (DIRECT INSTAGRAM) */}
          ...
          {/* PROFIL / AVATAR */}
          <button onClick={() => setIsProfileOpen(true)} className="w-7 h-7 rounded-full overflow-hidden border border-white">
            {user?.user_metadata?.avatar_url ? (
